...
(function () {
  ...
  if (modelCanvas) {
    Promise.all([
      ...
      // Используем встроенную процедурную модель (без внешних зависимостей)
      console.log('🔧 Используем встроенную 3D модель шатуна');

      // Сразу создаём fallback-модель
  ...
  /* --- Метрики: блик следует за курсором --------------------------------- */
  if (!reduced && window.matchMedia('(hover: hover)').matches) {
    var metricPending = null;
    document.addEventListener('pointermove', function (e) {
      if (metricPending) return;
      metricPending = requestAnimationFrame(function () {
        metricPending = null;
        var card = e.target.closest ? e.target.closest('.metric-card') : null;
        if (!card) return;
  ...
  /* --- История FERES: таймлайн с максимум 4 датами + навигация стрелками --- */
  var historyTimeline = document.querySelector('.history-timeline');
  if (historyTimeline) {
    var events = [
      { year: 1899, shortLabel: 'Основание', desc: 'Основание Federal‑Mogul.' },
      { year: 1929, shortLabel: 'Исследование', desc: 'Основание исследовательского подразделения.' },
      { year: 1998, shortLabel: 'Приобретение', desc: 'Приобретение активов компании Cooper (бренды Champion, Anco, MOOG, Wagner и др.).' },
      { year: 2003, shortLabel: 'Награды', desc: 'Награды Automotive News PACE Awards (16 наград с 2003 по 2016 год).' },
      { year: 2014, shortLabel: 'Реструктуризация', desc: 'Реструктуризация на два подразделения: Federal‑Mogul Powertrain и Federal‑Mogul Motor Parts.' },
      { year: 2017, shortLabel: 'Слияние', desc: 'Слияние с компанией Tenneco Inc.' },
      { year: 2022, shortLabel: 'Выход в СНГ', desc: 'Начало производства автокомпонентов под брендом Federal Reserve для рынка России и стран СНГ.' },
      { year: 2026, shortLabel: 'Ребрендинг', desc: 'Ребрендинг компании, регистрация товарной марки FERES. Поставка автозапчастей на вторичный рынок под маркой FERES.' }
    ];

    var yearsContainer = historyTimeline.querySelector('[data-timeline-years]');
    var labelsContainer = historyTimeline.querySelector('[data-timeline-labels]');
    var eventPanel = document.querySelector('.history__event');
    var prevButton = historyTimeline.querySelector('.timeline-bar__arrow--prev');
    var nextButton = historyTimeline.querySelector('.timeline-bar__arrow--next');

    var activeIndex = 0;
    var windowStart = 0; // Начало видимого окна (0, 1, 2, 3 для 5 событий на экран)
    var itemsPerPage = 5; // Ровно 5 видимых пунктов
    var maxWindow = Math.max(0, events.length - itemsPerPage); // Максимальное значение windowStart

    var renderTimeline = function () {
      // Видимые события в текущем окне
      var visibleEvents = events.slice(windowStart, windowStart + itemsPerPage);

      // Рендер годов
      if (yearsContainer) {
        yearsContainer.innerHTML = visibleEvents.map(function (evt, i) {
          var isActive = (windowStart + i) === activeIndex;
          return '<button class="timeline-bar__year' + (isActive ? ' is-active' : '') + '" data-index="' + (windowStart + i) + '">' + evt.year + '</button>';
        }).join('');
      }

      // Рендер подписей
      if (labelsContainer) {
        labelsContainer.innerHTML = visibleEvents.map(function (evt, i) {
          var isActive = (windowStart + i) === activeIndex;
          return '<div class="timeline-bar__label' + (isActive ? ' is-active' : '') + '" data-index="' + (windowStart + i) + '">' + evt.shortLabel + '</div>';
        }).join('');
      }

      // Обновляем панель события
      if (eventPanel && activeIndex >= 0 && activeIndex < events.length) {
        var activeEvent = events[activeIndex];
        var eyebrow = eventPanel.querySelector('.history__event-eyebrow');
        var body = eventPanel.querySelector('.history__event-body');
        if (eyebrow) eyebrow.textContent = activeEvent.year + ' г.';
        if (body) body.textContent = activeEvent.desc;
        eventPanel.style.opacity = '1';
      } else {
        eventPanel.style.opacity = '0';
      }

      // Обновляем состояние кнопок
      if (prevButton) prevButton.disabled = windowStart === 0;
      if (nextButton) nextButton.disabled = windowStart >= maxWindow;
    };

    // Клик на стрелку влево (предыдущие 5 дат)
    if (prevButton) {
    ...
    // Клик на стрелку вправо (следующие 5 дат)
    if (nextButton) {
      nextButton.addEventListener('click', function () {
        if (windowStart < maxWindow) {
          windowStart++;
          renderTimeline();
        }
      });
    ...
    // Клик на годы или подписи (переключает активное событие)
    historyTimeline.addEventListener('click', function (e) {
      var btn = e.target.closest('.timeline-bar__year');
      var lbl = e.target.closest('.timeline-bar__label');
      if (btn || lbl) {
        var idx = parseInt((btn || lbl).getAttribute('data-index'), 10);
    ...
    document.addEventListener('keydown', function (e) {
      if (!historyTimeline.contains(document.activeElement)) return;
      if (e.key === 'ArrowLeft') {
        e.preventDefault();
  ...
  /* --- Blueprint Parallax эффект --- */
  var bpSection = document.querySelector('.bp-parallax');
  var bpBg = document.querySelector('.bp-parallax__bg');

  if (bpSection && bpBg && !window.matchMedia('(prefers-reduced-motion: reduce)').matches) {
    var handleBpScroll = function () {
      var rect = bpSection.getBoundingClientRect();
      var sectionHeight = bpSection.offsetHeight;
      var viewportHeight = window.innerHeight;
      var scrollable = sectionHeight - viewportHeight;

      if (scrollable <= 0) return;

      // progress: 0 когда секция входит → 1 когда уходит
      var progress = Math.min(1, Math.max(0, -rect.top / scrollable));

      // чертёж смещается от 0% до -50% (едет вверх медленнее страницы)
      var yShift = progress * -50;
      bpBg.style.transform = 'translateX(-50%) translateY(' + yShift + '%)';
    };

    window.addEventListener('scroll', handleBpScroll, { passive: true });
    handleBpScroll(); // инициализация
  }
})();
